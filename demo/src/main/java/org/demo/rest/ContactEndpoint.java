...
public class ContactEndpoint
{
   ...
   {
      em.persist(entity);
      return Response.created(UriBuilder.fromResource(ContactEndpoint.class).path(String.valueOf(entity.getId())).build()).build();
   }
   ...
   @GET
   @Produces("application/json")
   public List<Contact> listAll(@QueryParam("start") Integer startPosition, @QueryParam("max") Integer maxResult)
   {
      TypedQuery<Contact> findAllQuery = em.createQuery("SELECT DISTINCT c FROM Contact c ORDER BY c.id", Contact.class);
      ...
      final List<Contact> results = findAllQuery.getResultList();
      return results;
   }
